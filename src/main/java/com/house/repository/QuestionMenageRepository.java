...
@Query("select SUM(combienPersonHandicapeEnfant) from QuestionMenageEntity p JOIN HouseHoldEntitty  h ON p.idMenage=h.id  where h.idExercise=?1 and p.idTrimestre=?2")
Integer nombreEnfantHandicape(Integer idExercice, Integer trimestre);

@Query("select SUM(combienPersonHandicapeJeunesse) from QuestionMenageEntity p JOIN HouseHoldEntitty  h ON p.idMenage=h.id  where h.idExercise=?1 and p.idTrimestre=?2")
